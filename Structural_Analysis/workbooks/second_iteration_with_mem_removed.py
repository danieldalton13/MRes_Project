...
class Truss:
    ...
    def remove_low_force_members(self, threshold=3):
        new_bars = []
        new_result = []
        for i, bar in enumerate(self.bars):
            if abs(self.result[i]) >= threshold:
                new_bars.append(bar)
                new_result.append(self.result[i])

        # Update bars and results
        self.bars = new_bars
        self.result = np.array(new_result)
...
class PgTruss:
    ...
    def update_truss(self, threshold=3):
        self.truss.remove_low_force_members(threshold)
        self.truss.solve()
...
t1.solve()
t1.remove_low_force_members(threshold=3)
t1.print_summary()

trussdraw = PgTruss(t1, 1600)
trussdraw.update_truss(threshold=3)
trussdraw.drawNodes()
