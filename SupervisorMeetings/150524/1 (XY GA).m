...
TolX = 0.1; % Tolerance for node - amount of +/- movement in x
TolY = 0.2; % Tolerance for node - amount of +/- movement in y
...
plot(NODE(movableNodesX, 1), NODE(movableNodesX, 2), 'ro', 'MarkerSize', 10); % Highlight movable nodes in x direction
plot(NODE(movableNodesY, 1), NODE(movableNodesY, 2), 'mo', 'MarkerSize', 10); % Highlight movable nodes in y direction

% Highlight nodes movable in both x and y directions in yellow
movableNodesXY = intersect(movableNodesX, movableNodesY);
plot(NODE(movableNodesXY, 1), NODE(movableNodesXY, 2), 'yo', 'MarkerSize', 10); % Highlight movable nodes in both directions

% Highlight support nodes in blue
supportNodes = SUPP(:, 1);
plot(NODE(supportNodes, 1), NODE(supportNodes, 2), 'bo', 'MarkerSize', 10); % Highlight support nodes

% Highlight load nodes in green
loadNodes = LOAD(:, 1);
plot(NODE(loadNodes, 1), NODE(loadNodes, 2), 'go', 'MarkerSize', 10); % Highlight load nodes
...
fitnessFcn = @(xy) evaluateStructure(xy, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, Lvl, RestrictDomain, ColTol, kappa);
...
function vol = evaluateStructure(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, Lvl, RestrictDomain, ColTol, kappa)
