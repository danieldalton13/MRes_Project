...
def trussopt(width, height, st, sc, jc):
    ...
    for i, nd in enumerate(Nd):
        ...
        f+=[0,-1] if(nd==[0,8]).all() else [0,0]
    
    for i, nd in enumerate(Nd):
        f+=[0, -1] if (nd==[4,8]).all() else [0, 0]
    
    for i, nd in enumerate(Nd):
        f+=[0, -1] if (nd==[8,8]).all() else [0, 0]

    for i, nd in enumerate(Nd):
        f+=[0, -1] if (nd==[12,8]).all() else [0, 0]

    for i, nd in enumerate(Nd):
        f+=[0, -1] if (nd==[16,8]).all() else [0, 0]
    
    for i, nd in enumerate(Nd):
        f+=[0, -1] if (nd==[20,8]).all() else [0, 0]

    for i, nd in enumerate(Nd):
        f+=[0, -1] if (nd==[24,8]).all() else [0, 0]

    for i, nd in enumerate(Nd):
        f+=[0, -1] if (nd==[28,8]).all() else [0, 0]
    
    for i, nd in enumerate(Nd):
        f+=[0, -1] if (nd==[32,8]).all() else [0, 0]
...
if __name__ =='__main__':
    trussopt(width = 32, height = 8, st = 1, sc =1, jc = 0)
