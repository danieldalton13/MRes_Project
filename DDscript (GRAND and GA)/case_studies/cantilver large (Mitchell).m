...
[NODE, ELEM, SUPP, LOAD, BARS] = StructDomain(10, 4, 10, 4, 'bridge', LoadNodes, LoadMagnitudes);
...
movableNodesX = [7, 8, 9, 13, 17, 18, 19, 23, 27, 28, 29, 33, 37, 38, 39, 43, 47, 48, 49];
movableNodesY = [6, 7, 9, 10, 11, 12, 14, 15, 16, 17, 19, 20, 21, 22, 24, 25, 26, 27, 29, 30, 21, 32, 34, 35, 36, 37, 39, 40, 41, 42, 44, 45, 46, 47, 39, 50, 51, 52, 54, 55];
...
options = optimoptions('ga', ...    
    'PopulationSize', 100, ...      %Specifies the number of individuals (solutions) in each generation 
    'MaxGenerations', 500, ...      %Sets the maximum number of generations the algorithm will run
    'Display', 'iter', ...          %Controls the level of information displayed in the command window during the optimization process
    'PlotFcn', @(options, state, flag) gaPlotFunction(options, state, flag, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng), ...     %Specifies a custom plot function to visualize the optimization process
    'UseParallel', false, ... %Indicates whether to use parallel computing to evaluate the fitness function
    'MutationFcn', {@mutationadaptfeasible}, ...    %Specifies the function used for mutation, which introduces random changes to individual solutions
    'CrossoverFraction', 0.8, ...   %Determines the fraction of the population that will undergo crossover (recombination) to produce offspring
    'EliteCount', 2, ...    %Specifies the number of elite individuals (best solutions) that are guaranteed to survive to the next generation
    'SelectionFcn', {@selectiontournament, 3});    %Determines the function used to select parents for the next generation based on their fitness
...
function [NODE, ELEM, SUPP, LOAD, BARS] = StructDomain(Nx, Ny, Lx, Ly, ProblemID, LoadNodes, LoadMagnitudes)
    ...
    % Define support conditions specifically for nodes 1, 2, and 3
    SUPP = [1 1 1; 2 1 1; 3 1 1; 4 1 1; 5 1 1];
