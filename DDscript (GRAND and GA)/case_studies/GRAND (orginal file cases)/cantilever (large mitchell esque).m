...
LoadNodes = [53]; % Nodes with applied loads
LoadMagnitudes = [0, -1]; % Load magnitudes (x, y) for each node
RestrainedNodes = [1, 2, 3, 4, 5];
Restraint = [
    1, 1;  % Node 1 restrained in both x and y
    1, 1;  % Node 2 restrained in both x and y
    1, 1;  % Node 3 restrained in both x and y
    1, 1;  % Node 4 restrained in both x and y
    1, 1;  % Node 5 restrained in both x and y
];
[NODE, ELEM, SUPP, LOAD] = StructDomain(10, 4, 10, 4, 'bridge', LoadNodes, LoadMagnitudes, RestrainedNodes, Restraint);
...
function [NODE, ELEM, SUPP, LOAD] = StructDomain(Nx, Ny, Lx, Ly, ProblemID, LoadNodes, LoadMagnitudes, RestrainedNodes, Restraint)
    ...
    % Initialize SUPP based on restrained nodes and their restraints
    SUPP = [];
    for i = 1:length(RestrainedNodes)
        node = RestrainedNodes(i);
        restraint = Restraint(i, :);
        SUPP = [SUPP; node, restraint];
    end

    if nargin < 4 || isempty(ProblemID)
        ProblemID = 1;
    end
    switch ProblemID
...
function [BARS] = GenerateGS(NODE, ELEM, Lvl, RestrictDomain, ColTol)
    ...
    for i = 2:Lvl
        ...
        if isempty(J)
            Lvl = i - 1; fprintf('-INFO- No new bars at Level %g\n', Lvl); break
        end
...
function [BC] = GetSupports(SUPP)
    ...
    Nf = sum(sum(SUPP(:, 2:3))); % Modified to sum only the binary restraint flags
    BC = zeros(Nf, 1); j = 0;
    for i = 1:size(SUPP, 1)
        if SUPP(i, 2) == 1
            j = j + 1; BC(j) = 2 * SUPP(i, 1) - 1;
        end
        if SUPP(i, 3) == 1
            j = j + 1; BC(j) = 2 * SUPP(i, 1);
        end
    end
    if j ~= Nf
        error('Parsing number mismatch on BCs.')
    end
end
