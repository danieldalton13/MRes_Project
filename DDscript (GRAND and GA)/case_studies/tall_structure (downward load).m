...
TolX = 0.1; % Tolerance for node movement in x
TolY = 0.1; % Tolerance for node movement in y
RestrictDomain = []; % No restriction for box domain

% Define the nodes where loads are applied and their respective magnitudes
LoadNodes = [5, 10, 15];
LoadMagnitudes = [1, 1, 1];

% Generate the structured orthogonal domain
[NODE, ELEM, SUPP, LOAD, BARS] = StructDomain(2, 4, 2, 5, 'bridge', LoadNodes, LoadMagnitudes);
