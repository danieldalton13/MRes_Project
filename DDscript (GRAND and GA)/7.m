...
% Calculate and print connection angles at each node
calculateAndPrintAngles(NODE, BARS);
...
function calculateAndPrintAngles(NODE, BARS)
    Nb = size(BARS, 1);
    for i = 1:size(NODE, 1)
        ...
        if numConnections > 1
            fprintf('Node %d:\n', i);
            for j = 1:numConnections-1
                for k = j+1:numConnections
                    n1 = BARS(connectedBars(j),:);
                    n2 = BARS(connectedBars(k),:);
                    vec1 = NODE(n1(1),:) - NODE(n1(2),:);
                    vec2 = NODE(n2(1),:) - NODE(n2(2),:);
                    angle = acosd(dot(vec1,vec2) / (norm(vec1) * norm(vec2)));
                    fprintf('  Angle between bar %d and bar %d: %.2f degrees\n', connectedBars(j), connectedBars(k), angle);
                end
            end
        end
    end
