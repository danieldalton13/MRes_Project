%% Ground Structure Optimization using GA and Non-linear Programming
...
alpha = 0; % Weighting factor for the squared sum of cross-sectional areas
beta = 10; % Weighting factor for the SSD term
...
options = optimoptions('ga', ...
    ...
    'MaxGenerations', 100, ... 
    'Display', 'iter', ...
    'PlotFcn', @(options, state, flag) gaPlotFunction(options, state, flag, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, alpha, beta), ...
    'UseParallel', false);

% Define the fitness function for the GA
fitnessFcn = @(xy) evaluateStructure(xy, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, alpha, beta);
...
% Calculate and print the final costs
[finalCost, finalVolume, finalSumAreas] = calculateCosts(NODE, BARS, SUPP, LOAD, kappa, alpha, beta);
fprintf('Final Cost: %.2f\n', finalCost);
fprintf('Final Total Structure Volume: %.2f\n', finalVolume);
fprintf('Final Sum of All Cross-Sectional Areas: %.2f\n', finalSumAreas);
...
function obj = objectiveFunction(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, BARS, SUPP, LOAD, kappa, alpha, beta)
    ...
    % Calculate the additional term (sum of cross-sectional areas)^2
    sumCrossSections = sum(A);
    penalty = alpha * (sumCrossSections^2);
    
    % Calculate the sum of squared differences in cross-sectional areas at nodes (SSD)
    SSD = calculateSSD(NODE, BARS, A);
    
    % Print the SSD value
    fprintf('Sum of Squared Differences (SSD): %.2f\n', SSD);
    
    % Calculate the total objective value
    obj = vol + penalty + beta * SSD;
end

% Function to calculate the sum of squared differences in cross-sectional areas at nodes
function SSD = calculateSSD(NODE, BARS, A)
    numNodes = size(NODE, 1); % Total number of nodes
    SSD = 0; % Initialize SSD
    
    fprintf('\nCalculating SSD...\n');
    
    % Loop through each node
    for i = 1:numNodes
        connectedBars = []; % Initialize the list of connected bars
        
        % Find all bars connected to the current node
        for j = 1:size(BARS, 1)
            if BARS(j, 1) == i || BARS(j, 2) == i
                connectedBars = [connectedBars; A(j)];
            end
        end
        
        % Exclude zero values for the SSD calculation
        nonZeroConnectedBars = connectedBars(connectedBars > 0);
        
        % Print the connected bars and their areas for this node
        if ~isempty(nonZeroConnectedBars)
            fprintf('Node %d connected bars areas: %s\n', i, num2str(nonZeroConnectedBars'));
        end
        
        % If there is more than one non-zero bar connected to the node, calculate the squared difference
        if length(nonZeroConnectedBars) > 1
            maxArea = max(nonZeroConnectedBars);
            minArea = min(nonZeroConnectedBars);
            difference = maxArea - minArea;
            squaredDifference = difference^2;
            SSD = SSD + squaredDifference;
            
            % Print the calculation details for this node
            fprintf('Node %d: maxArea = %.2f, minArea = %.2f, difference = %.2f, squaredDifference = %.2f\n', ...
                    i, maxArea, minArea, difference, squaredDifference);
        elseif length(nonZeroConnectedBars) == 1
            % If only one non-zero connected bar, the difference is zero
            fprintf('Node %d: Only one non-zero connected bar, difference = 0, squaredDifference = 0\n', i);
        end
    end
    
    % Print the final SSD value
    fprintf('Sum of Squared Differences (SSD): %.2f\n', SSD);
end

% Fitness function for GA
function score = evaluateStructure(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, alpha, beta)
    score = objectiveFunction(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, BARS, SUPP, LOAD, kappa, alpha, beta);
end

% Custom Plot Function for GA with modularization and consistent tolerances
function state = gaPlotFunction(options, state, flag, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, alpha, beta)
    ...
    sumCrossSections = sum(A);
    penalty = alpha * (sumCrossSections^2);
    cost = vol + penalty + beta * calculateSSD(NODE, BARS, A);
    ...
    % Print current cost and components
    fprintf('Generation %d: Cost = %.2f, Volume = %.2f, Sum of Areas = %.2f\n', state.Generation, cost, vol, sumCrossSections);
...
function visualizeNodes(NODE, SUPP, LOAD)
    ...
    % Highlight movable nodes in x (red), y (magenta), and both (yellow)
    highlightNodes(NODE, [5, 11, 17, 23], 'ro', 'Movable in X');
    highlightNodes(NODE, [3, 6, 9, 12, 15, 18, 21, 24, 27], 'mo', 'Movable in Y');
    highlightNodes(NODE, intersect([5, 11, 17, 23], [3, 6, 9, 12, 15, 18, 21, 24, 27]), 'yo', 'Movable in X and Y');

    % Highlight support nodes (blue) and load nodes (green)
    highlightNodes(NODE, SUPP(:, 1), 'bo', 'Supports');
    highlightNodes(NODE, LOAD(:, 1), 'go', 'Loads');
end

function highlightNodes(NODE, indices, color, label)
    plot(NODE(indices, 1), NODE(indices, 2), color, 'MarkerSize', 10, 'DisplayName', label);
...
function memberInfo = PlotGroundStructure(NODE, BARS, A, Cutoff, Ng, SUPP)
    ...
    MyGroup = ceil(Ng * A(ind) / max(A(ind))); % Group bars by their areas
    ...
    for i = Ng:-1:1 % Plot each group of bars
        if ~isempty(Groups{i})
            for j = 1:length(Groups{i})
                ...
                plot(XY(:, [1 3])', XY(:, [2 4])', 'LineWidth', 5 * sqrt(GroupArea / max(A)), 'Color', cmap(i, :))
                % Store member information
                memberInfo{end+1} = struct('Start', NODE(BARS(barIndex, 1), :), ...
                                           'End', NODE(BARS(barIndex, 2), :), ...
                                           'CrossArea', round(GroupArea, 2));
            end
        end
    end
...
function anglesAtNodes = calculateAndPrintAngles(NODE, memberInfo)
    ...
    for i = 1:size(NODE, 1)
        ...
        if numConnections == 1
            ...
            % Store angle sets
            angleSets{end+1} = strcat(num2str(round(sort(angle), 2)), '-', mat2str(round(crossArea, 2)));
        elseif numConnections > 1
            ...
            [~, crossAreaSortIdx] = sortrows(crossAreas);
            crossAreas = crossAreas(crossAreaSortIdx, :);
    ...
    for i = 1:length(anglesAtNodes)
        if ~isempty(anglesAtNodes(i).Angles)
            if anglesAtNodes(i).Angles == 360.00
                ...
                fprintf('Node %d: %.2f', anglesAtNodes(i).Node, anglesAtNodes(i).Angles(1));
                for j = 2:length(anglesAtNodes(i).Angles)
                    fprintf(', %.2f', anglesAtNodes(i).Angles(j));
                end
                fprintf('\n');
                fprintf('Node %d (cross-area): %.2f', anglesAtNodes(i).Node, anglesAtNodes(i).CrossAreas(1));
...
function angleSetCounts = countAngleSetOccurrencesWithTolerance(anglesAtNodes, tolerance)
    ...
    function existingKey = findSimilarKey(map, newKey, tolerance)
        ...
        keys = map.keys();
        for i = 1:length(keys)
            existingAngles = str2num(keys{i}); %#ok<ST2NM>
...
function angleSetCountsIncludingAreas = countAngleSetOccurrencesIncludingAreasWithTolerance(anglesAtNodes, angleTolerance, areaTolerance)
    ...
    function isSimilar = isSimilar(value1, value2, tolerance)
    ...
    function existingKey = findSimilarKey(map, newKey, angleTolerance, areaTolerance)
        ...
        keys = map.keys();
        for i = 1:length(keys)
            existingAngles = str2num(keys{i}(1:strfind(keys{i}, '-') - 1)); %#ok<ST2NM>
            newAngles = str2num(newKey(1:strfind(newKey, '-') - 1)); %#ok<ST2NM>
            if length(existingAngles) == length(newAngles)
                similar = true;
                for j = 1:length(existingAngles)
                    if ~isSimilar(existingAngles(j), newAngles(j), angleTolerance)
                        similar = false;
                        break;
                    end
                end
                existingAreas = str2num(keys{i}(strfind(keys{i}, '-') + 1:end)); %#ok<ST2NM>
                newAreas = str2num(newKey(strfind(newKey, '-') + 1:end)); %#ok<ST2NM>
                if length(existingAreas) == length(newAreas)
                    for j = 1:length(existingAreas)
                        if ~isSimilar(existingAreas(j), newAreas(j), areaTolerance)
                            similar = false;
                            break;
                        end
                    end
...
% Function to calculate the cost, total structure volume, and sum of all member cross-sectional areas
function [cost, totalVolume, sumCrossSections] = calculateCosts(NODE, BARS, SUPP, LOAD, kappa, alpha, beta)
    [vol, exitflag, A] = solveLP(NODE, BARS, SUPP, LOAD, kappa);
    if exitflag ~= 1
        vol = inf; % Penalize infeasible solutions
    end
    sumCrossSections = sum(A);
    penalty = alpha * (sumCrossSections^2);
    SSD = calculateSSD(NODE, BARS, A); % Calculate the SSD value
    cost = vol + penalty + beta * SSD;
    totalVolume = vol;
end
