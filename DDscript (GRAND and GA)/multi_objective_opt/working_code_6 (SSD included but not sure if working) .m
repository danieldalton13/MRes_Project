...
% and fmincon for material usage minimization.
...
% Prepare matrix for non-linear programming
BTBT = [BT -BT]; 
LL = [L; kappa * L]; 
sizeBTBT = whos('BTBT'); 
clear BT L;
...
options = optimoptions('ga', ...
    ...
    'MaxGenerations', 100, ... 
    'Display', 'iter', ...
    'PlotFcn', @(options, state, flag) gaPlotFunction(options, state, flag, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, alpha, beta, Nb, BTBT, LL, F), ...
    'UseParallel', false);

% Define the fitness function for the GA
fitnessFcn = @(xy) evaluateStructure(xy, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, alpha, beta, BTBT, LL, F);
...
[finalCost, finalVolume, finalSumAreas, finalSSD] = calculateCosts(NODE, BARS, SUPP, LOAD, kappa, alpha, beta);
fprintf('Final Cost: %.2f\n', finalCost);
fprintf('Final Total Structure Volume: %.2f\n', finalVolume);
fprintf('Final Sum of All Cross-Sectional Areas: %.2f\n', finalSumAreas);
fprintf('Final Sum of Squared Differences (SSD): %.2f\n', finalSSD);
...
function obj = objectiveFunction(S, LL, BTBT, F, NODE, BARS, kappa, alpha, beta)
    % Separate slack variables
    S = reshape(S, numel(S) / 2, 2); % Separate slack variables
    A = S(:, 1) + kappa * S(:, 2); % Get cross-sectional areas
    
    % Ensure LL and A dimensions match for element-wise multiplication
    if size(LL, 1) ~= length(A)
        LL = LL(1:length(A)); % Adjust LL length to match A
    end
    ...
    fprintf('Current Objective Value: %.2f, Volume: %.2f, Penalty: %.2f, SSD: %.2f\n', obj, vol, penalty, SSD);
end

% Fitness function for GA
function score = evaluateStructure(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, alpha, beta, BTBT, LL, F)
    ...
    % Solve the non-linear programming problem to find optimal cross-sectional areas
    ...
    % Calculate the total objective value
    score = vol + penalty + beta * SSD;
    
    % Print intermediate results
    fprintf('Intermediate Score: %.2f, Volume: %.2f, Penalty: %.2f, SSD: %.2f\n', score, vol, penalty, SSD);
end

% Custom Plot Function for GA with modularization and consistent tolerances
function state = gaPlotFunction(options, state, flag, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, alpha, beta, Nb, BTBT, LL, F)
    ...
    [S, vol, exitflag] = fmincon(@(S) objectiveFunction(S, LL, BTBT, F, NODE, BARS, kappa, alpha, beta), zeros(2 * Nb, 1), [], [], BTBT, F, zeros(2 * Nb, 1), [], [], options);
    if exitflag ~= 1
        return;
    end
    ...
    drawnow;
    pause(0.1); % Add a short pause to ensure the figure updates

    % Print current cost and components
    fprintf('Generation %d: Cost = %.2f, Volume = %.2f, Sum of Areas = %.2f, SSD = %.2f\n', state.Generation, cost, vol, sumCrossSections, calculateSSD(NODE, BARS, A));
end
...
function [cost, totalVolume, sumCrossSections, SSD] = calculateCosts(NODE, BARS, SUPP, LOAD, kappa, alpha, beta)
    [vol, exitflag, A] = solveLP(NODE, BARS, SUPP, LOAD, kappa);
    if exitflag ~= 1
        vol = inf; % Penalize infeasible solutions
    end
...
% Function to calculate the sum of squared differences (SSD) in cross-sectional areas at nodes
function SSD = calculateSSD(NODE, BARS, A)
    SSD = 0;
    for i = 1:size(NODE, 1)
        connectedBars = find(BARS(:, 1) == i | BARS(:, 2) == i);
        if ~isempty(connectedBars)
            areas = A(connectedBars);
            nonZeroAreas = areas(areas > 0);
            if length(nonZeroAreas) > 1
                maxArea = max(nonZeroAreas);
                minArea = min(nonZeroAreas);
                SSD = SSD + (maxArea - minArea)^2;
                fprintf('Node %d connected bars areas: %s\n', i, mat2str(nonZeroAreas));
                fprintf('Node %d: maxArea = %.2f, minArea = %.2f, difference = %.2f, squaredDifference = %.2f\n', ...
                    i, maxArea, minArea, maxArea - minArea, (maxArea - minArea)^2);
            end
        end
    end
