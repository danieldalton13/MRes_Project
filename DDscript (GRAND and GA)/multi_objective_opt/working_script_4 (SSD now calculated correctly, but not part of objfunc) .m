...
alpha = 0.1; % Weighting factor for the squared sum of cross-sectional areas
...
options = optimoptions('ga', ...
    ...
    'MaxGenerations', 100, ... 
    'Display', 'iter', ...
    'PlotFcn', @(options, state, flag) gaPlotFunction(options, state, flag, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, alpha), ...
    'UseParallel', false);

% Define the fitness function for the GA
fitnessFcn = @(xy) evaluateStructure(xy, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, alpha);
...
[finalCost, finalVolume, finalSumAreas] = calculateCosts(NODE, BARS, SUPP, LOAD, kappa, alpha);
...
function obj = objectiveFunction(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, BARS, SUPP, LOAD, kappa, alpha)
...
function score = evaluateStructure(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, alpha)
    score = objectiveFunction(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, BARS, SUPP, LOAD, kappa, alpha);
end

% Custom Plot Function for GA with modularization and consistent tolerances
function state = gaPlotFunction(options, state, flag, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, alpha)
...
function [cost, totalVolume, sumCrossSections] = calculateCosts(NODE, BARS, SUPP, LOAD, kappa, alpha)
    [vol, exitflag, A] = solveLP(NODE, BARS, SUPP, LOAD, kappa);
    if exitflag ~= 1
        vol = inf; % Penalize infeasible solutions
    end
