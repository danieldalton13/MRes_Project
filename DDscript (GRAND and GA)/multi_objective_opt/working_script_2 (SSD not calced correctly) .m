...
beta = 0.1; % Weighting factor for the SSD term
...
function obj = objectiveFunction(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, BARS, SUPP, LOAD, kappa, alpha, beta)
    ...
    SSD_penalty = beta * SSD;
    
    % Calculate the total objective value
    obj = vol + penalty + SSD_penalty;
end
...
function memberInfo = PlotGroundStructure(NODE, BARS, A, Cutoff, Ng, SUPP)
    ...
    % No normalization for areas
    ind = find(A > Cutoff); % Find indices of bars with area above cutoff
    MyGroup = ceil(Ng * A(ind) / max(A)); % Group bars by their areas
    ...
    for i = Ng:-1:1 % Plot each group of bars
        if ~isempty(Groups{i})
            for j = 1:length(Groups{i})
                ...
                GroupArea = A(barIndex); % Use actual area for this group
