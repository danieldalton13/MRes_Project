...
function [cost, exitflag, A, vol, memberInfo] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa)
    try
        ...
        [S, ~, exitflag] = fmincon(@(S) objectiveFunction(S, LL), initialS, [], [], BTBT, F, zeros(2 * Nb, 1), inf(2 * Nb, 1), [], options);
        ...
        if exitflag == 1
            ...
            totalArea = sum(A);
            cost = totalArea^2 + vol; % Compute the total cost
...
function stop = outputFcn(x, optimValues, state, NODE, BARS)
    ...
    switch state
        case 'iter'
            ...
            % Print cross-sectional areas around each node and calculate sum of squared differences
...
function obj = objectiveFunction(S, LL)
    % Calculate the volume term
    vol = LL' * S;
    % Calculate the total cross-sectional area
    A = sum(S(1:length(S)/2) + S(length(S)/2+1:end));
