...
function SSD = calculateSSD(NODE, BARS, A)
    ...
    for i = 1:numNodes
        ...
        if ~isempty(connectedBars)
            fprintf('Node %d connected bars areas: %s\n', i, num2str(connectedBars'));
        end
        
        % If there is more than one bar connected to the node, calculate the squared difference
        if length(connectedBars) > 1
            maxArea = max(connectedBars);
            minArea = min(connectedBars);
