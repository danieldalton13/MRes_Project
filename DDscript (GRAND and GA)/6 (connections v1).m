...
% Count unique node connections in the final optimized structure
uniqueConnections = countUniqueNodeConnections(NODE, BARS);
fprintf('Number of unique node connections: %d\n', uniqueConnections);
...
% Function to count unique node connections in the final optimized structure
function uniqueConnectionCount = countUniqueNodeConnections(NODE, BARS)
    % Initialize a cell array to store connection patterns at each node
    connectionsAtNodes = cell(size(NODE, 1), 1);
    
    % Iterate through each bar to define connections
    for i = 1:size(BARS, 1)
        % Get the nodes connected by the bar
        n1 = BARS(i, 1);
        n2 = BARS(i, 2);
        
        % Calculate the relative position of n2 with respect to n1
        vec = NODE(n2, :) - NODE(n1, :);
        angle = atan2(vec(2), vec(1)); % Calculate angle of the bar
        
        % Store the connection information at both nodes
        connectionsAtNodes{n1} = [connectionsAtNodes{n1}; angle];
        connectionsAtNodes{n2} = [connectionsAtNodes{n2}; angle + pi]; % Opposite direction
    end
    
    % Normalize the connection patterns by sorting and generating canonical forms
    uniquePatterns = containers.Map('KeyType', 'char', 'ValueType', 'double');
    for i = 1:length(connectionsAtNodes)
        if ~isempty(connectionsAtNodes{i})
            % Normalize the connection pattern
            pattern = normalizePattern(connectionsAtNodes{i});
            patternStr = mat2str(pattern);
            
            % Add the canonical pattern to the map
            if ~isKey(uniquePatterns, patternStr)
                uniquePatterns(patternStr) = 1;
            end
        end
    end
    
    % The number of unique patterns is the number of keys in the map
    uniqueConnectionCount = length(uniquePatterns);
end

function normalizedPattern = normalizePattern(pattern)
    % Sort the angles
    pattern = sort(mod(pattern, 2*pi));
    
    % Generate all rotations and reflections
    allPatterns = {};
    for theta = 0:pi/2:(2*pi - pi/2)
        rotated = mod(pattern + theta, 2*pi);
        reflected = mod(2*pi - rotated, 2*pi);
        allPatterns{end+1} = mat2str(sort(rotated));
        allPatterns{end+1} = mat2str(sort(reflected));
    end
    
    % Find the minimum string representation
    normalizedPattern = allPatterns{1};
    for i = 2:length(allPatterns)
        if strcmp(allPatterns{i}, normalizedPattern)
            normalizedPattern = allPatterns{i};
        end
    end
end
