% Objective Function = w1.Volume + w2.PenaltyConstant
...
options = optimoptions('ga', ...
    ...
    'MaxGenerations', 100, ... 
    'Display', 'iter', ...
    'PlotFcn', @(options, state, flag) gaPlotFunction(options, state, flag, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, w1, w2, penaltyConstant), ...
    'UseParallel', false);

% Define the fitness function for the GA
fitnessFcn = @(xy) evaluateStructure(xy, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, w1, w2, penaltyConstant);
...
function state = gaPlotFunction(options, state, flag, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, w1, w2, penaltyConstant)
    ...
    [vol, exitflag, A] = solveLP(NODE, BARS, SUPP, LOAD, kappa, w1, w2, penaltyConstant);
    if exitflag ~= 1
        return;
    end
...
function [vol, exitflag, A] = solveLP(NODE, BARS, SUPP, LOAD, kappa, w1, w2, penaltyConstant)
    try
        Nn = size(NODE, 1); % Number of nodes
        Nb = size(BARS, 1); % Number of bars
        ...
        % Solve the linear programming problem to find optimal cross-sectional areas
        [S, vol, exitflag] = linprog(w1 * LL, [], [], BTBT, F, zeros(2 * Nb, 1));
        vol = vol + w2 * penaltyConstant; % Adjust objective function with penalty
        if exitflag == 1
...
function score = evaluateStructure(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, w1, w2, penaltyConstant)
    ...
    % Solve the linear programming problem
    [vol, exitflag, ~] = solveLP(NODE, BARS, SUPP, LOAD, kappa, w1, w2, penaltyConstant);
    if exitflag ~= 1
