...
options = optimoptions('ga', ...
    'PopulationSize', 50, ...
    'MaxGenerations', 50, ... % Reduced number of generations
    'Display', 'iter', ...
...
function state = gaPlotFunction(options, state, flag, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng)
    ...
    BTBT = [BT -BT];
    LL = [L; kappa * L];
    [S, ~, exitflag] = linprog(LL, [], [], BTBT, F, zeros(2 * Nb, 1));
    if exitflag ~= 1
    ...
    % Set the angle tolerance
    angleTolerance = 3.0; % Tolerance of 3.0 degrees
    areaTolerance = 1.0; % Tolerance of 1.0 for cross-sectional areas
...
% Fitness function to evaluate structure
