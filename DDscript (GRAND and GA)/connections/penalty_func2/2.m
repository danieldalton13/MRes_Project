% Start of an implementation of a penalty functio for cross sections but doesnt have great influence
...
TolX = 0.0; % Tolerance for node movement in x
TolY = 0.0; % Tolerance for node movement in y
...
movableNodesY = [5, 11, 17, 23, 29, 35];
...
function visualizeNodes(NODE, SUPP, LOAD)
    ...
    plot(NODE(movableNodesY, 1), NODE(movableNodesY, 2), 'go', 'MarkerSize', 10); 
    ...
    loadNodes = LOAD(:, 1);
    plot(NODE(loadNodes, 1), NODE(loadNodes, 2), 'co', 'MarkerSize', 10); 

    % Add a legend
    legend({'Nodes', 'Movable in X (Red)', 'Movable in Y (Green)', 'Movable in Both (Yellow)', 'Supports (Blue)', 'Loads (Cyan)'}, 'Location', 'BestOutside');
end
...
function score = evaluateStructure(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa)
    ...
    % Calculate penalty
    penalty = calculatePenalty(NODE, BARS, A);
    
    % Total score is the volume plus the penalty
    score = vol + penalty;
end
...
% Function to calculate the penalty for cross-sectional area differences at nodes
function penalty = calculatePenalty(NODE, BARS, A)
    penalty = 0;
    nodeConnections = cell(size(NODE, 1), 1);
    for i = 1:size(BARS, 1)
        node1 = BARS(i, 1);
        node2 = BARS(i, 2);
        nodeConnections{node1} = [nodeConnections{node1}, i];
        nodeConnections{node2} = [nodeConnections{node2}, i];
    end

    for i = 1:length(nodeConnections)
        if length(nodeConnections{i}) > 1
            crossSections = A(nodeConnections{i});
            penalty = penalty + var(crossSections);
        end
    end
end
