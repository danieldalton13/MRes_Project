%% Ground Structure Optimization using GA and Nonlinear Programming
% This script optimizes a ground structure problem using GA for node displacement
% and nonlinear programming for material usage minimization with an additional cost term.

%% Parameters
% Optimization parameters
alpha = 1.0; % Coefficient for SSD term
beta = 1.0; % Coefficient for Volume term
...
fitnessFcn = @(xy) nonlinearObjective(xy, movableNodesX, movableNodesY, NODE, BARS, SUPP, LOAD, kappa, alpha, beta);

% Run the genetic algorithm to find the best node positions
[bestXY, bestCost] = ga(fitnessFcn, numVars, [], [], [], [], lb, ub, [], options);
...
% Solve the nonlinear structure problem with the updated NODE positions
[cost, ~, A, vol, memberInfo, SSD] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa, alpha, beta);

% Calculate the total cost with the additional term
totalCost = alpha * SSD + beta * vol;

% Print the final cost, volume, and total area
fprintf('Final Cost: %f\n', totalCost);
fprintf('Final Volume: %f\n', vol);

% Plot the final ground structure with cross-sectional areas
figure;
PlotGroundStructure(NODE, BARS, A, Cutoff, Ng, SUPP, memberInfo);
title('Optimized Structure');

% Print member information
for i = 1:length(memberInfo)
    if memberInfo{i}.CrossArea > 0
...
function state = gaPlotFunction(options, state, flag, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, alpha, beta)
    ...
    % Solve the nonlinear structure problem
    [~, ~, A, vol, memberInfo, SSD] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa, alpha, beta);

    % Update the plot
    figure(figHandle);
    clf;
    PlotGroundStructure(NODE, BARS, A, Cutoff, Ng, SUPP, memberInfo);
    title(sprintf('Optimization Process: Generation %d', state.Generation));
    drawnow;
...
% Function to solve the nonlinear structure problem
function [cost, exitflag, A, vol, memberInfo, SSD] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa, alpha, beta)
    try
        ...
        % Prepare matrix for nonlinear programming
        BTBT = [BT -BT];
        LL = [L; kappa * L];

        % Solve the nonlinear programming problem to find optimal cross-sectional areas
        options = optimoptions('fmincon', ...
            'Algorithm', 'interior-point', ...
            'Display', 'iter', ...
            'OutputFcn', @(x, optimValues, state) outputFcn(x, optimValues, state, NODE, BARS)); % Include custom output function

        initialS = zeros(2 * Nb, 1); % Initial guess
        [S, ~, exitflag] = fmincon(@(S) objectiveFunction(S, LL, alpha, beta), initialS, [], [], BTBT, F, zeros(2 * Nb, 1), inf(2 * Nb, 1), [], options);
        ...
        if exitflag == 1
            ...
            vol = LL' * [S(:, 1); S(:, 2)]; % Compute the volume
            SSD = 0; % Initialize SSD

            % Store member information
            memberInfo = cell(Nb, 1);
            for i = 1:Nb
                memberInfo{i} = struct('Start', NODE(BARS(i, 1), :), ...
                                       'End', NODE(BARS(i, 2), :), ...
                                       'CrossArea', A(i));
            end
            ...
            for i = 1:length(angleSets)
                angleSet = angleSets{i};
                count = angleSetCounts(angleSet);
                SSD = SSD + (count - mean(count))^2; % Calculate SSD
            end

            cost = alpha * SSD + beta * vol; % Compute the total cost
        else
            A = [];
            vol = inf;
            cost = inf;
        ...
        warning('Error occurred in solveNonlinearStructure: %s', ME.message);
        cost = inf;
        exitflag = -1;
        A = [];
        vol = inf;
...
% Output function to track the iterations of fmincon
function stop = outputFcn(x, optimValues, state, NODE, BARS)
    stop = false;
    Cutoff = 0.05; % Define the cutoff value for cross-sectional areas
    persistent SSD;
    ...
    switch state
        ...
        case 'iter'
            ...
            A = x(1:end/2) + x(end/2+1:end); % Compute cross-sectional areas
            A = A / max(A); % Normalize areas

            % Group cross-sectional areas by nodes
            nodeAreas = containers.Map('KeyType', 'char', 'ValueType', 'any');
            for i = 1:length(A)
                if A(i) > Cutoff
                    startNode = BARS(i, 1);
                    endNode = BARS(i, 2);

                    % Add cross-sectional area to the start node
                    startCoord = NODE(startNode, :);
                    startCoordStr = sprintf('(%.2f, %.2f)', startCoord(1), startCoord(2));
                    if isKey(nodeAreas, startCoordStr)
                        nodeAreas(startCoordStr) = [nodeAreas(startCoordStr), A(i)];
                    else
                        nodeAreas(startCoordStr) = A(i);
                    end

                    % Add cross-sectional area to the end node
                    endCoord = NODE(endNode, :);
                    endCoordStr = sprintf('(%.2f, %.2f)', endCoord(1), endCoord(2));
                    if isKey(nodeAreas, endCoordStr)
                        nodeAreas(endCoordStr) = [nodeAreas(endCoordStr), A(i)];
                    else
                        nodeAreas(endCoordStr) = A(i);
                    end
                end
            end

            % Calculate and print cross-sectional areas around each node and calculate SSD
            SSD = 0;
            nodeKeys = keys(nodeAreas);
            for k = 1:length(nodeKeys)
                nodeStr = nodeKeys{k};
                areas = nodeAreas(nodeStr);
                fprintf('%s - {%.2f', nodeStr, areas(1));
                for j = 2:length(areas)
                    fprintf(', %.2f', areas(j));
                end
                fprintf('}\n');

                % Calculate the difference between the largest and smallest cross-sectional areas
                maxArea = max(areas);
                minArea = min(areas);
            ...
            fprintf('Sum of Squared Differences: %.6f\n', SSD);
    end
end

% Objective function for the nonlinear problem
function obj = objectiveFunction(S, LL, alpha, beta)
    % Calculate the volume term
    vol = LL' * S;
    % Get the persistent SSD value from the output function
    persistent SSD;
    if isempty(SSD)
        SSD = 0;
    end
    % Define the cost function
    obj = alpha * SSD + beta * vol;
end

% Function to evaluate the structure
function cost = nonlinearObjective(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, BARS, SUPP, LOAD, kappa, alpha, beta)
    ...
    % Solve the nonlinear structure problem
    [cost, ~, ~, ~, ~, ~] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa, alpha, beta);
end
...
function PlotGroundStructure(NODE, BARS, A, Cutoff, Ng, SUPP, memberInfo)
    hold on, axis equal, axis off;

    if isempty(A)
        warning('No bars to plot.');
        return;
    end
...
function angleSetCounts = countAngleSetOccurrencesWithTolerance(anglesAtNodes, tolerance)
    ...
    function existingKey = findSimilarKey(map, newKey, tolerance)
        ...
        keys = map.keys();
        for i = 1:length(keys)
            existingAngles = str2double(split(keys{i})); %#ok<ST2NM>
            newAngles = str2double(split(newKey)); %#ok<ST2NM>
...
function angleSetCountsIncludingAreas = countAngleSetOccurrencesIncludingAreasWithTolerance(anglesAtNodes, angleTolerance, areaTolerance)
    ...
    function existingKey = findSimilarKey(map, newKey, angleTolerance, areaTolerance)
        ...
        keys = map.keys();
        for i = 1:length(keys)
            keyParts = split(keys{i}, '-');
            existingAngles = str2double(split(keyParts{1})); %#ok<ST2NM>
            newKeyParts = split(newKey, '-');
            newAngles = str2double(split(newKeyParts{1})); %#ok<ST2NM>
            if length(existingAngles) == length(newAngles)
                similar = true;
                for j = 1:length(existingAngles)
                ...
                existingAreas = str2double(split(keyParts{2}, ';')); %#ok<ST2NM>
                newAreas = str2double(split(newKeyParts{2}, ';')); %#ok<ST2NM>
    ...
    for i = 1:length(anglesAtNodes)
        if ~isempty(anglesAtNodes(i).Angles)
            ...
            angleSetStr = strcat(num2str(sortedAngles), '-', num2str(sortedCrossAreas', '%.8f;'));
