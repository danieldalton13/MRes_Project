...
options = optimoptions('ga', ...
    ...
    'MaxGenerations', 100, ...
    'Display', 'iter', ...
    'PlotFcn', @(options, state, flag) gaPlotFunction(options, state, flag, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng), ...
    'UseParallel', false);

% Define the fitness function for the GA
fitnessFcn = @(xy) nonlinearObjective(xy, movableNodesX, movableNodesY, NODE, BARS, SUPP, LOAD, kappa);
...
[~, ~, A, vol, memberInfo] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa);

% Calculate the total area of all bars
totalArea = sum(A);

% Calculate the total cost with the SSD term
totalCost = vol + calculateSSD(A, NODE, BARS);
...
function state = gaPlotFunction(options, state, flag, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng)
    ...
    [~, vol, A, ~, memberInfo] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa);

    % Calculate SSD
    SSD = calculateSSD(A, NODE, BARS);
    ...
    % Print cost, volume, and SSD
    fprintf('Generation %d: Cost = %.6f, Volume = %.6f, SSD = %.6f\n', state.Generation, totalCost, vol, SSD);
...
function [cost, vol, A, exitflag, memberInfo] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa)
    try
        ...
        [S, ~, exitflag] = fmincon(@(S) objectiveFunction(S, LL, NODE, BARS), initialS, [], [], BTBT, F, zeros(2 * Nb, 1), inf(2 * Nb, 1), [], options);
        ...
        if exitflag == 1
            ...
            cost = vol + calculateSSD(S, NODE, BARS); % Compute the total cost including SSD
...
function stop = outputFcn(x, optimValues, state, NODE, BARS)
    ...
    switch state
        case 'iter'
            ...
            fprintf('Iteration %d: Cost = %.6f, Volume = %.6f, SSD = %.6f\n', optimValues.iteration, optimValues.fval, optimValues.fval - calculateSSD(x, NODE, BARS), calculateSSD(x, NODE, BARS));
    end
end

% Objective function for the nonlinear problem
function obj = objectiveFunction(S, LL, NODE, BARS)
    % Calculate the volume term
    vol = LL' * S;
    
    % Calculate the SSD term
    SSD = calculateSSD(S, NODE, BARS);
    
    % Define the cost function including SSD
    obj = vol + SSD;
end

% Function to calculate SSD
function SSD = calculateSSD(S, NODE, BARS)
    A = S(1:end/2) + S(end/2+1:end); % Compute cross-sectional areas
    ...
    for k = 1:length(nodeKeys)
        areas = nodeAreas(nodeKeys{k});
        maxArea = max(areas);
        minArea = min(areas);
        difference = maxArea - minArea;
        squaredDifference = difference^2;
        sumOfSquaredDifferences = sumOfSquaredDifferences + squaredDifference;
    end
...
function cost = nonlinearObjective(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, BARS, SUPP, LOAD, kappa)
    ...
    % Solve the nonlinear structure problem
    [cost, ~, ~, ~, ~] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa);
end
