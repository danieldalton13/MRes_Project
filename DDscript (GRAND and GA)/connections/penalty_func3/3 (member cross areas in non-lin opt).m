...
function [cost, exitflag, A, vol, memberInfo] = solveNonlinearStructure(NODE, BARS, SUPP, LOAD, kappa)
    try
        ...
        options = optimoptions('fmincon', 'Algorithm', 'interior-point', 'Display', 'off', 'OutputFcn', @outputFcn); % Change algorithm
...
function stop = outputFcn(x, optimValues, state)
    ...
    switch state
        case 'iter'
            ...
            A = x(1:end/2) + x(end/2+1:end);
            fprintf('Cross-sectional areas: %s\n', mat2str(A));
    end
end
