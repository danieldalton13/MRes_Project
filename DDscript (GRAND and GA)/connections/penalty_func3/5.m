...
function stop = outputFcn(x, optimValues, state, NODE, BARS)
    ...
    switch state
        case 'iter'
            ...
            % Prepare string to display non-zero cross-sectional areas with coordinates
            areaCoordsStr = '';
            for i = 1:length(A)
                if A(i) > 0
                    startNode = BARS(i, 1);
                    ...
                    areaCoordsStr = sprintf('%s%.2f {(%.2f, %.2f), (%.2f, %.2f)};\n', areaCoordsStr, A(i), startCoord(1), startCoord(2), endCoord(1), endCoord(2));
                end
            end
            
            fprintf('Cross-sectional areas with coordinates:\n%s', areaCoordsStr);
    end
end
