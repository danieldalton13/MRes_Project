...
% Count occurrences of different angle sets
angleSetCounts = countAngleSetOccurrences(anglesAtNodes);
...
% Count occurrences of different angle sets including cross-sectional areas
tolerance = 0.1;
angleSetCountsIncludingAreas = countAngleSetOccurrencesIncludingAreas(anglesAtNodes, tolerance);
...
function anglesAtNodes = calculateAndPrintAngles(NODE, memberInfo)
    ...
    for i = 1:size(NODE, 1)
        ...
        if numConnections == 1
            ...
            % Store angles and colors in the structure
            anglesAtNodes(i).Node = i;
            anglesAtNodes(i).Angles = angle;
            anglesAtNodes(i).Colors = colors;
            
            % Store angle sets
            angleSets{end+1} = strcat(num2str(sort(angle)), '-', mat2str(round(colors, 2)));
        elseif numConnections > 1
            ...
            % Sort angles and their corresponding colors together
            [angles, sortIdx] = sort(angles, 'ascend');
            colors = colors(sortIdx, :);
            
            % Sort colors based on numerical values
            [~, colorSortIdx] = sortrows(colors);
            colors = colors(colorSortIdx, :);
            ...
            if ~all(angleDiffs == 180.00)
                ...
                % Store angle sets
                angleSetStr = strcat(num2str(angleDiffs), '-', mat2str(round(colors, 2)));
    ...
    for i = 1:length(anglesAtNodes)
        if ~isempty(anglesAtNodes(i).Angles)
            if anglesAtNodes(i).Angles == 360.00
                ...
                fprintf('Node %d (color): [%.2f, %.2f, %.2f]\n', anglesAtNodes(i).Node, anglesAtNodes(i).Colors(1), anglesAtNodes(i).Colors(2), anglesAtNodes(i).Colors(3));
            else
                ...
                fprintf('\n');
                fprintf('Node %d (color): [%.2f, %.2f, %.2f]', anglesAtNodes(i).Node, anglesAtNodes(i).Colors(1, 1), anglesAtNodes(i).Colors(1, 2), anglesAtNodes(i).Colors(1, 3));
                for j = 2:size(anglesAtNodes(i).Colors, 1)
                    fprintf(', [%.2f, %.2f, %.2f]', anglesAtNodes(i).Colors(j, 1), anglesAtNodes(i).Colors(j, 2), anglesAtNodes(i).Colors(j, 3));
                end
                fprintf('\n');
            end
        end
...
% Function to count occurrences of different angle sets including cross-sectional areas with tolerance
function angleSetCountsIncludingAreas = countAngleSetOccurrencesIncludingAreas(anglesAtNodes, tolerance)
    angleSetCountsIncludingAreas = containers.Map('KeyType', 'char', 'ValueType', 'int32');
    
    function isSimilar = isSimilarColor(color1, color2, tolerance)
        isSimilar = all(abs(color1 - color2) <= tolerance);
    end
    ...
    function existingKey = findSimilarKey(map, newKey, tolerance)
        ...
        keys = map.keys();
        for i = 1:length(keys)
            existingColors = str2num(keys{i}(strfind(keys{i}, '-') + 1:end)); %#ok<ST2NM>
            newColors = str2num(newKey(strfind(newKey, '-') + 1:end)); %#ok<ST2NM>
            if size(existingColors, 1) == size(newColors, 1)
                similar = true;
                for j = 1:size(existingColors, 1)
                    if ~isSimilarColor(existingColors(j, :), newColors(j, :), tolerance)
                        similar = false;
                        break;
                    end
                end
    ...
    for i = 1:length(anglesAtNodes)
        if ~isempty(anglesAtNodes(i).Angles)
            % Create a sorted key of angles and colors
            [sortedAngles, sortIdx] = sort(anglesAtNodes(i).Angles);
            sortedColors = anglesAtNodes(i).Colors(sortIdx, :);
            % Sort colors based on numerical values
            [~, colorSortIdx] = sortrows(sortedColors);
            sortedColors = sortedColors(colorSortIdx, :);
            angleSetStr = strcat(num2str(sortedAngles), '-', mat2str(round(sortedColors, 2)));
            
            existingKey = findSimilarKey(angleSetCountsIncludingAreas, angleSetStr, tolerance);
...
% Function to count occurrences of different angle sets
function angleSetCounts = countAngleSetOccurrences(anglesAtNodes)
    angleSetCounts = containers.Map('KeyType', 'char', 'ValueType', 'int32');
    for i = 1:length(anglesAtNodes)
        if ~isempty(anglesAtNodes(i).Angles)
            angleSetStr = num2str(sort(anglesAtNodes(i).Angles));
            if isKey(angleSetCounts, angleSetStr)
                angleSetCounts(angleSetStr) = angleSetCounts(angleSetStr) + 1;
            else
