...
% Calculate and print angles at each node
...
function memberInfo = PlotGroundStructure(NODE, BARS, A, Cutoff, Ng, SUPP)
    ...
    for i = 1:size(SUPP, 1)
        plot(NODE(SUPP(i, 1), 1), NODE(SUPP(i, 1), 2), '^', 'MarkerSize', 8, 'MarkerFaceColor', 'm');
    end
...
% Function to calculate and print angles at each node
function anglesAtNodes = calculateAndPrintAngles(NODE, memberInfo)
    anglesAtNodes = struct(); % Initialize struct to store angles at each node
    for i = 1:size(NODE, 1)
        connectedBars = [];
        for j = 1:length(memberInfo)
            if all(NODE(i, :) == memberInfo{j}.Start) || all(NODE(i, :) == memberInfo{j}.End)
        ...
        if numConnections > 1
            ...
            for j = 1:numConnections
                barIndex = connectedBars(j);
                if all(NODE(i, :) == memberInfo{barIndex}.Start)
                    otherPos = memberInfo{barIndex}.End;
                else
                    otherPos = memberInfo{barIndex}.Start;
                end
            ...
            % Store and print angles
            anglesAtNodes.(sprintf('Node%d', i)) = angleDiffs;
