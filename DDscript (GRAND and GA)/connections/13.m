...
options = optimoptions('ga', ...
    ...
    'MaxGenerations', 50, ... 
    'Display', 'iter', ...
    'PlotFcn', @(options, state, flag) gaPlotFunction(options, state, flag, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, originalNODE, penaltyWeight, angleTolerance, areaTolerance), ...
    'UseParallel', false);

% Define the fitness function for the GA
penaltyWeight = 10; % Define a suitable penalty weight
originalNODE = NODE; % Save the original positions of nodes
angleTolerance = 1.0; % Define angle tolerance
areaTolerance = 0.2; % Define area tolerance

fitnessFcn = @(xy) evaluateStructure(xy, movableNodesX, movableNodesY, NODE, originalNODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, penaltyWeight, angleTolerance, areaTolerance, Cutoff, Ng);
...
function state = gaPlotFunction(options, state, flag, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng, originalNODE, penaltyWeight, angleTolerance, areaTolerance)
    ...
    [vol, exitflag, A] = solveLP(NODE, originalNODE, BARS, SUPP, LOAD, kappa, penaltyWeight, angleTolerance, areaTolerance, Cutoff, Ng);
    if exitflag ~= 1
        return;
    end
...
% Function to solve the linear programming problem with error handling and penalty
function [vol, exitflag, A] = solveLP(NODE, originalNODE, BARS, SUPP, LOAD, kappa, penaltyWeight, angleTolerance, areaTolerance, Cutoff, Ng)
    try
        Nn = size(NODE, 1); % Number of nodes
        Nb = size(BARS, 1); % Number of bars
        ...
        if exitflag == 1
            ...
            memberInfo = PlotGroundStructure(NODE, BARS, A, Cutoff, Ng, SUPP);
            numTypes = calculateTotalDifferentTypesOfNodes(NODE, memberInfo, angleTolerance, areaTolerance);
            penalty = nodeDiversityPenalty(numTypes);

            % Add the penalty to the objective function value
            vol = vol + penalty * penaltyWeight;
        else
            A = [];
        end
    catch ME
...
function score = evaluateStructure(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, originalNODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, penaltyWeight, angleTolerance, areaTolerance, Cutoff, Ng)
    ...
    % Solve the linear programming problem
    [vol, exitflag, ~] = solveLP(NODE, originalNODE, BARS, SUPP, LOAD, kappa, penaltyWeight, angleTolerance, areaTolerance, Cutoff, Ng);
    if exitflag ~= 1
...
% Function to calculate total different types of nodes
function count = calculateTotalDifferentTypesOfNodes(NODE, memberInfo, angleTolerance, areaTolerance)
    anglesAtNodes = calculateAndPrintAngles(NODE, memberInfo);
    angleSetCountsIncludingAreas = countAngleSetOccurrencesIncludingAreasWithTolerance(anglesAtNodes, angleTolerance, areaTolerance);
    count = length(angleSetCountsIncludingAreas);
end

% Penalty function for node diversity
function penalty = nodeDiversityPenalty(numTypes)
    penalty = (1/10) * numTypes^2;
end
