...
%% Determine connection types and print results
uniqueConnectionTypes = getUniqueConnectionTypes(NODE, BARS, A, Cutoff);
fprintf('Final volume: %f\n', vol);
disp('Number of unique connection types:');
disp(length(uniqueConnectionTypes));
...
% Function to determine unique connection types
function uniqueConnectionTypes = getUniqueConnectionTypes(NODE, BARS, A, Cutoff)
    % Find bars with cross-sectional area above cutoff
    ind = find(A > Cutoff);
    activeBars = BARS(ind, :);
    
    % Initialize containers for connection types
    connectionTypes = containers.Map('KeyType', 'char', 'ValueType', 'int32');
    
    % Iterate through each node to find connections
    for i = 1:size(NODE, 1)
        % Find bars connected to node i
        connectedBars = activeBars(any(activeBars == i, 2), :);
        if isempty(connectedBars)
            continue;
        end
        
        % Determine angles between connected members
        numBars = size(connectedBars, 1);
        angles = zeros(numBars, 1);
        nodeCoord = NODE(i, :);
        for j = 1:numBars
            % Get the other node of the bar
            otherNode = connectedBars(j, connectedBars(j, :) ~= i);
            otherCoord = NODE(otherNode, :);
            % Compute angle with respect to the horizontal
            angles(j) = atan2(otherCoord(2) - nodeCoord(2), otherCoord(1) - nodeCoord(1));
        end
        
        % Sort angles to avoid redundancy
        angles = sort(mod(angles, 2 * pi));
        anglesDiff = diff(angles);
        anglesDiff = [anglesDiff; 2 * pi - sum(anglesDiff)];
        
        % Generate a unique key for the connection type
        key = sprintf('%d|', numBars);
        for k = 1:numBars
            key = strcat(key, sprintf('%.2f', anglesDiff(k)));
        end
        
        % Add the key to the set of connection types
        if ~isKey(connectionTypes, key)
            connectionTypes(key) = 1;
        end
    end
    
    % Convert the map to a list of unique connection types
    uniqueConnectionTypes = keys(connectionTypes);
end
