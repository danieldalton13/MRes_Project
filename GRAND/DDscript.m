...
function [x] = BndryCnds(Node, Element, BdBox)
    ...
    QuarterSpan = sqrt((Node(:,1)-(BdBox(1)+0.25*(BdBox(2)-BdBox(1)))).^2 + (Node(:,2)-BdBox(3)).^2);
    [~, QuarterSpan] = sort(QuarterSpan);
    ThreeQuarterSpan = sqrt((Node(:,1)-(BdBox(1)+0.75*(BdBox(2)-BdBox(1)))).^2 + (Node(:,2)-BdBox(3)).^2);
    [~, ThreeQuarterSpan] = sort(ThreeQuarterSpan);

    % Apply loads at specific spans
    Load = [
        QuarterSpan(1), 0, -1;      % Load at 1/4 span
        MidSpan(1), 0, -1;          % Load at midspan
        ThreeQuarterSpan(1), 0, -1  % Load at 3/4 span
    ];
