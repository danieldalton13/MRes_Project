%% Ground Structure Optimization using PSO and Linear Programming
% This script optimizes a ground structure problem using PSO for node displacement 
...
figure;
visualizeNodes(NODE, SUPP, LOAD, movableNodesX, movableNodesY);
...
if numVars > 0
    ...
    options = optimoptions('particleswarm', ...
        ...
        'MaxIterations', 100, ...
        'Display', 'iter', ...
        'PlotFcn', @(optimValues, state) psoPlotFunction(optimValues, state, movableNodesX, movableNodesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng));

    % Run the PSO to find the best node positions
    [bestXY, bestVol] = particleswarm(fitnessFcn, numVars, lb, ub, options);
    ...
    fprintf('No movable nodes defined. Skipping PSO optimization.\n');
end
...
% Display the number of nodes, bars, and elements in the final optimized structure
...
% Check if totalVolumes has been populated
if ~isempty(totalVolumes)
    figure;
    ...
    warning('totalVolumes is empty. No data to plot for Total Volume vs. Iterations.');
end
...
% PSO Plot Function
function stop = psoPlotFunction(optimValues, state, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng)
    stop = false;
    persistent figHandle;
    global totalVolumes; % Ensure we are accessing the global variable
    ...
    if exitflag ~= 1
        vol = inf; % Assign infinity if solution is infeasible
    end

    % Update the global totalVolumes array
    ...
    title(sprintf('Optimization Process: Iteration %d', optimValues.iteration));
    drawnow;
end
...
function score = evaluateStructure(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa)
    % Update NODE coordinates with PSO variables
