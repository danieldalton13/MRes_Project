%% Ground Structure Optimization using Simulated Annealing and Linear Programming
% This script optimizes a ground structure problem using Simulated Annealing for node displacement 
...
Cutoff = 0.02; % Minimum cross-sectional area for plotting
Ng = 50; % Number of groups for color coding bars in plots
TolX = 1; % Tolerance for node movement in x
TolY = 1; % Tolerance for node movement in y
RestrictDomain = []; % No restriction for box domain

% Define the nodes where loads are applied and their respective magnitudes and directions
LoadNodes = [26];
LoadMagnitudes = [1];
LoadDirections = [0, -1]; % [x, y] directions for each load node

% Define the support nodes
SupportNodes = [1, 2, 3]; % Nodes specified as supports

% Generate the structured orthogonal domain
[NODE, ELEM, SUPP, LOAD, BARS] = StructDomain(8, 2, 8, 2, 'bridge', LoadNodes, LoadMagnitudes, LoadDirections, SupportNodes);

% Count initial number of nodes, elements, and bars
initialNumNodes = size(NODE, 1);
initialNumBars = size(BARS, 1);
initialNumElements = length(ELEM);

fprintf('Initial Structure: %d Nodes, %d Bars, %d Elements\n', initialNumNodes, initialNumBars, initialNumElements);
...
movableNodesY = [4,6,7,9,10,12,13,15,16,18,19,21,22,24,25,27];
...
if numVars > 0
    ...
    initialTemp = 30.0; % Initial temperature
    coolingRate = 0.95; % Cooling rate

    % Run the Simulated Annealing to find the best node positions
    [bestXY, bestVol] = simulatedAnnealing(fitnessFcn, initialXY, lb, ub, maxIter, initialTemp, coolingRate);
    ...
    fprintf('No movable nodes defined. Skipping Simulated Annealing optimization.\n');
end
...
xlabel('Iterations');
ylabel('Total Volume');
title('Total Volume vs. Iterations');
...
% Filter out bars with negligible cross-sectional areas
optimizedBars = BARS(A > Cutoff, :);
optimizedNumBars = size(optimizedBars, 1);
optimizedNumElements = length(ELEM); % Assuming elements count doesn't change

fprintf('Optimized Structure: %d Nodes, %d Bars, %d Elements\n', initialNumNodes, optimizedNumBars, optimizedNumElements);
...
function visualizeNodes(NODE, SUPP, LOAD, movableNodesX, movableNodesY)
    ...
    if ~isempty(movableNodesX) && ~isempty(movableNodesY)
    ...
    % Highlight load nodes and draw arrows for the loads
    loadNodes = LOAD(:, 1);
    plot(NODE(loadNodes, 1), NODE(loadNodes, 2), 'go', 'MarkerSize', 10); 
    quiver(NODE(loadNodes, 1), NODE(loadNodes, 2), LOAD(:, 2), LOAD(:, 3), 0.5, 'r');
end

%% Simulated Annealing Function
function [bestXY, bestVol] = simulatedAnnealing(fitnessFcn, initialXY, lb, ub, maxIter, initialTemp, coolingRate)
    global totalVolumes;
    currentXY = initialXY;
    currentVol = fitnessFcn(currentXY);
    bestXY = currentXY;
    bestVol = currentVol;
    temp = initialTemp;

    perturbationScale = 0.5; % Scale for perturbation size

    for iter = 1:maxIter
        % Generate a new candidate solution with larger perturbation
        perturbation = perturbationScale * (ub - lb) .* (rand(size(currentXY)) - 0.5);
        newXY = currentXY + perturbation;
        newXY = max(min(newXY, ub), lb); % Ensure within bounds
        newVol = fitnessFcn(newXY);

        % Accept new solution with probability based on temperature
        if newVol < currentVol || rand() < exp((currentVol - newVol) / temp)
            currentXY = newXY;
            currentVol = newVol;
        ...
        % Store the volume for the current iteration
        totalVolumes = [totalVolumes, bestVol];

        % Decrease temperature
        temp = temp * coolingRate;

        % Reduce perturbation scale over time to fine-tune
        perturbationScale = perturbationScale * 0.99;
    end
end
...
function score = evaluateStructure(xy, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa)
    % Update NODE coordinates with Simulated Annealing variables
...
function [NODE, ELEM, SUPP, LOAD, BARS] = StructDomain(Nx, Ny, Lx, Ly, ProblemID, LoadNodes, LoadMagnitudes, LoadDirections, SupportNodes)
    ...
    % Define support conditions based on SupportNodes
    SUPP = [SupportNodes', ones(length(SupportNodes), 1), ones(length(SupportNodes), 1)];

    % Apply loads with specified magnitudes and directions
    LOAD = zeros(length(LoadNodes), 3); % Initialize LOAD matrix
    for i = 1:length(LoadNodes)
        LOAD(i, :) = [LoadNodes(i), LoadMagnitudes(i) * LoadDirections(i, 1), LoadMagnitudes(i) * LoadDirections(i, 2)];
    end
...
% Total running time
totalRunningTime = toc(startTime);
