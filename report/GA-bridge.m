...
% Number of nodes in the starting structure
initialNumNodes = size(NODE, 1);
fprintf('Number of Nodes in the Starting Structure: %d\n', initialNumNodes);

% Number of elements in the starting structure
initialNumElements = length(ELEM);
fprintf('Number of Elements in the Starting Structure: %d\n', initialNumElements);

% Number of elements in the final structure
finalNumElements = sum(A > Cutoff);
fprintf('Number of Elements in the Final Structure: %d\n', finalNumElements);

% Total running time
totalRunningTime = toc(startTime);
fprintf('Total Running Time: %f seconds\n', totalRunningTime);

%% Print GA Crossover and Mutation Rates
% Get the crossover fraction (crossover rate)
crossoverRate = options.CrossoverFraction;

% Get the mutation function
mutationFcn = options.MutationFcn;

% Display the crossover rate
