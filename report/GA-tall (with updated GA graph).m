...
% Initialize global variable to store volume history
global VolumeHistory;
VolumeHistory = []; % Clear any existing history
...
% Define the nodes that will act as supports
SupportNodes = [1, 19]; % Specify support nodes

% Generate the structured orthogonal domain
[NODE, ELEM, SUPP, LOAD, BARS] = StructDomain(2, 8, 2, 8, 'bridge', LoadNodes, LoadMagnitudes, LoadDirections, SupportNodes);

% Define movable nodes for optimization
movableNodesX = []; 
movableNodesY = [11, 13, 15, 17];

% Visualize node numbering with loads
figure;
visualizeNodes(NODE, SUPP, LOAD, movableNodesX, movableNodesY);
title('Node Numbering with Loads');
...
if numVars > 0
    ...
    fprintf('No movable nodes defined. Skipping GA optimization.\n');
end
...
%% Plot Total Volume vs Generations
global VolumeHistory;
figure;
plot(1:length(VolumeHistory), VolumeHistory, '-o');
xlabel('Generation');
ylabel('Total Volume');
...
function state = gaPlotFunction(options, state, flag, MovableNodeIndicesX, MovableNodeIndicesY, NODE, ELEM, SUPP, LOAD, BARS, ColTol, kappa, Cutoff, Ng)
    ...
    VolumeHistory = [VolumeHistory; vol];
...
function [NODE, ELEM, SUPP, LOAD, BARS] = StructDomain(Nx, Ny, Lx, Ly, ProblemID, LoadNodes, LoadMagnitudes, LoadDirections, SupportNodes)
    ...
    % Initialize SUPP matrix based on SupportNodes
    SUPP = zeros(length(SupportNodes), 3);
    for i = 1:length(SupportNodes)
        SUPP(i, :) = [SupportNodes(i), 1, 1]; % Full support (both x and y restrained)
    end

    % Initialize LOAD matrix
    LOAD = zeros(length(LoadNodes), 3);
